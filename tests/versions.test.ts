...
describe('versions', () => {
  ...
  describe('versions', () => {
    ...
    it('should have all expected builtin versions', () => {
      ...
      expect(versions.schemas['full']).toBe('26')
      expect(versions.schemas['lite']).toBe('9')
      expect(versions.schemas['superpowers-lite']).toBeUndefined()
      expect(versions.skills['systematic-debugging']).toBe('1.0.2')
      expect(versions.skills['automated-instrumented-debugging']).toBeUndefined()
      expect(versions.skills['brainstorming']).toBe('9.1.1')
      expect(versions.skills['workflow-routing']).toBe('1.1.1')
      expect(versions.skills['grilling']).toBeUndefined()
      expect(versions.skills['requirement-analysis']).toBe('3.1.1')
      expect(versions.skills['technical-design']).toBe('3.1.1')
      ...
      expect(versions.skills['openspec-new-change']).toBe('2.4')
      expect(versions.skills['openspec-propose']).toBeUndefined()
      expect(versions.skills['openspec-ff-change']).toBe('2.2')
      expect(versions.skills['openspec-explore']).toBeUndefined()
      expect(versions.skills['openspec-onboard']).toBe('2.4')
      expect(versions.skills['openspec-continue-change']).toBe('2.2')
      expect(versions.skills['openspec-update-change']).toBe('2.2')
      expect(versions.skills['openspec-apply-change']).toBe('3.2')
      expect(versions.skills['openspec-archive-change']).toBe('3.3')
      expect(versions.skills['openspec-bulk-archive-change']).toBe('2.2')
      expect(versions.skills['openspec-sync-specs']).toBe('2.2')
      expect(versions.skills['openspec-verify-change']).toBe('3.4')
