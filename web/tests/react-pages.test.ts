...
describe('React pages', () => {
  ...
  it('keeps the main, DevKeel research, and V1 sharing decks as standalone static pages', () => {
    ...
    const loopHandoffIndex = sharingSource.indexOf('阶段交接 · Harness → Loop')
    ...
    expect(sharingSource).toContain('阶段交接 · Context → Harness')
    expect(sharingSource).toContain('阶段交接 · Harness → Loop')
    expect(sharingSource).toContain('↺ 回写 Harness · 下一次任务自动继承')
  ...
  it('keeps themed architecture diagrams accessible and responsive', () => {
    ...
    expect(source).toContain('alt="渐进工作流：核对缺口，按需进入 brainstorming；Lite 讨论聚焦补缺，Full 讨论完成双探针检查，讨论深度可独立于 Direct、Lite、Full 工作流调整"')
  ...
  it('keeps website and design-source diagrams on the same topology', () => {
    ...
    const nodeIds = (svg: string): string[] => [...svg.matchAll(/data-node-id="([^"]+)"/g)].map((match) => match[1])
    const edgeIds = (svg: string): string[] => [...svg.matchAll(/data-edge-id="([^"]+)"/g)].map((match) => match[1])
    expect(nodeIds(darkProgressiveSource)).toEqual(nodeIds(progressiveSource))
    expect(edgeIds(darkProgressiveSource)).toEqual(edgeIds(progressiveSource))
    ...
    for (const source of [progressiveSource, darkProgressiveSource]) {
      ...
      expect(source).toContain('/brainstorming')
      expect(source).toContain('需求探针')
      expect(source).toContain('技术探针')
      expect(source).toContain('阻塞缺口合并去重')
      expect(source).toContain('每轮只讨论一个关键决定')
      expect(source).toContain('给出推荐、依据与代价')
      expect(source).toContain('有关键缺口或显式要求时进入')
      expect(source).toContain('Lite 讨论 · 围绕当前方向补缺')
      expect(source).toContain('按需调用需求或技术探针')
      expect(source).toContain('Full 讨论 · 检查假设、替代方向与风险')
      expect(source).toContain('完成双探针检查，可复用有效结论')
      expect(source).toContain('检查完成且无阻塞项')
      expect(source).toContain('经确认保存共同设计')
      expect(source).toContain('经确认进行完整风险治理')
      expect(source).toContain('id="interview-to-rank"')
      expect(source).toContain('id="task-to-agent"')
      expect(source).toContain('id="agent-to-gap"')
      expect(source).toContain('id="gap-to-evidence"')
    }
