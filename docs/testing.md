...
Web 的 `static-site.test.ts` 实际构建 `/` 和 `/devkeel/` 两种产物，检查当前页面、V1 兼容入口、
资源路径、静态版本目录和跳转页，并验证无需 API 服务的本地开发模式。`routes.test.ts`
校验前缀边界；构建还会核对 public 与 dist 中的原始版本文件集合及内容一致。
