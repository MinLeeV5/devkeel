...
`templates/` 目录 SHALL 包含 `package.json`，`name` 为 `devkeel-templates`，通过 `files` 字段白名单控制发布内容，`publishConfig.registry` 指向公共 npm registry。
...
#### Scenario: publishConfig 使用公共 registry
