# 《从 Prompt 到 Loop》V2 分享讲稿

> 版本：2026-08-04
> 正文时长：约 30 分钟；Q&A：10 分钟
> 配套页面：`web/public/sharing.html`  
> 案例说明：手机登录对话与命令输出均为教学示意；两页真实项目案例来自已检查的仓库结构与提交事实。

## 01. 从 Prompt 到 Loop（1 分钟）

今天不讲一组 Prompt 技巧，而是讲 AI Agent 的工程环境。我们会用“给现有登录页增加手机号验证码登录”这个普通需求，观察 Prompt、Context、Harness、Loop 分别解决什么问题。
...
- Harness 解决“Agent 是否会按项目方式执行并交出证据”。
...
## 10. 交接：Context → Harness（1.7 分钟）

Context 阶段的交付物是 `AGENTS.md` 项目地图。它保持短小，按任务指向登录实现、短信契约、前端规则和验证入口。

**Context 的 DONE WHEN：** Agent 能定位并按需加载完成任务所需的事实。

它仍然回答不了三个系统问题：谁选择 Direct、Lite、Full；谁执行授权和质量门禁；谁判断完成并留下证据。Harness 接手的正是“编排、约束、证据”。

主分享接下来直接进入 Harness。Agent Skills、Superpowers 与 Matt Skills 的调研证据已拆到独立补充页 `sharing-harness-research.html`，需要展开设计来源时再切换。

## 11. Harness：可靠执行环境（1.5 分钟）

Harness 不是另一份 Prompt 模板。最小模型是：
...
Harness V1 默认把 brainstorm、design、specs、tasks、apply、verify、retrospective 和 archive 串成完整链路。V2 保留“先澄清、再实施、验证后完成”的工程底线，但不再让每个任务支付相同流程成本。
...
Harness 不是文件清单；关键是这些载体能被发现、按需加载并形成反馈闭环。

## 17. 手机登录的 Lite 闭环（1.4 分钟）

手机登录涉及多个交互状态、复用约束和验收证据，因此值得留下简短协调记录；后端契约不变，也没有迁移或高后果风险，所以不需要 Full。
...
这是一个普通的 Vite Todo 项目，起点只有 React、TypeScript、Vite、Zustand 和已有 feature 目录。`devkeel init` 建立统一入口，`domain-init` 从真实代码补出 8 条项目规则与 1 个前端 reviewer，业务结构没有被重排。
...
前后端合计沉淀了 41 条 rules 与 18 个 skills。Agent 不必一次吞下整个产品：根仓负责找到边界，子仓负责提供领域知识、专项能力和反馈命令；只有需要跨会话协调的工作才进入 OpenSpec。

**核心句：** 同一套 Harness 不要求同一种仓库形态，它让知识跟着 ownership 和验证边界生长。

## 20. 交接：Harness → Loop（1.4 分钟）

Harness 阶段的交付物是一个 **Agent-ready Repository**：任务可路由、可约束、可验证、可审查。
...
内环解决当前任务：目标、行动、验证、纠偏或停止。外环改善执行系统：交付、复盘、提炼、更新 Harness，让下一次任务自动继承。

Loop 不是无限重试。它必须有可判定信号、修正策略、停止条件和人类升级入口。

## 22. 四阶段最终对比（1.1 分钟）

Prompt 优化单次表达，Context 优化当下认知，Harness 优化执行可靠性，Loop 优化系统学习率。
...
返工不断先查 Prompt；方案与项目脱节先查 Context；完成无法复跑先查 Harness；同类问题反复出现则查 Loop。

**收束句：** 真正的升级，不是让 Agent 更会回答，而是让团队更会交付。

## 24. Q&A（0.5 分钟，之后预留 10 分钟）

谢谢大家。页面上是 Harness 的安装方式、文档站、GitLab 和交流群。
