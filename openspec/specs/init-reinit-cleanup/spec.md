...
When generating a new `package.json`, the setup script MUST include submodule initialization.

#### Scenario: new project with submodules
- **WHEN** init creates a new package.json
- **THEN** the `setup` script SHALL be `git submodule update --remote --init`
